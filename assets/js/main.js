...
function applyTheme(theme, opts){
  ...
  updateMetaTheme();
  // A11y: announce theme changes in a non-intrusive way
...
if (prefersDarkMql){
  const onPrefersChange = (evt) => {
    if (readStoredTheme()) return; // don't override explicit choice
...
addEventListener('DOMContentLoaded', setHeaderOffset);
...
// Initialize menu hidden state & a11y attributes
if (langMenu) {
...
// TDZ FIX: declare these BEFORE any use
let langButtons = [];
let langSearchTerm = '';
...
const fallbackLangs = [
  ...
  // Added 5 new high-impact languages
  { code: 'he', english: 'Hebrew', native: 'עברית', dir: 'rtl' },
  { code: 'ko', english: 'Korean', native: '한국어', dir: 'ltr' },
  { code: 'el', english: 'Greek', native: 'Ελληνικά', dir: 'ltr' },
  { code: 'vi', english: 'Vietnamese', native: 'Tiếng Việt', dir: 'ltr' },
  { code: 'id', english: 'Indonesian', native: 'Bahasa Indonesia', dir: 'ltr' }
];
...
// Optional sanitizer: allow minimal inline markup from translations
const sanitizer = ('Sanitizer' in window)
  ? new window.Sanitizer({ allowElements: ['strong','em','br','a'] })
  : null;
...
function applyTranslations(payload) {
  ...
  for (const el of i18nElements) {
    ...
    if (el.innerHTML !== html) {
      if (sanitizer && el.setHTML) {
        el.setHTML(html, { sanitizer });
      } else {
        // Note: keep your translation sources trusted; this path doesn't sanitize.
        el.innerHTML = html;
      }
    }
  }
